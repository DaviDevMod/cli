...
class Npm {
  ...
  async #load () {
    ...
    // we get the chalk level based on a null stream meaning chalk will
    // only use what it knows about the environment to get color support
    // since we already determined in our definitions if the stdout/stderr
    // streams are supporting colors
    const { level } = createSupportsColor(null)
